...
describe('share view URLs', () => {
  it('parses a viewport with selected route and search query', () => {
    expect(parseSharedView('?lat=-33.8688&lng=151.2093&z=9.5&route=r-1&q=Sydney')).toEqual({
      lat: -33.8688,
      lng: 151.2093,
      z: 9.5,
      route: 'r-1',
  ...
  it('builds a share link with viewport, selected node, and query', () => {
    const url = buildSharedViewURL('https://routes.australiaverse.org/?old=1&route=old', { lat: -37.8136, lng: 144.9631, z: 8.125 }, {
      node: 'node-123',
      q: 'Melbourne'
    });
    expect(url).toBe('https://routes.australiaverse.org/?old=1&lat=-37.8136&lng=144.9631&z=8.13&node=node-123&q=Melbourne');
  });
